...
export function makeApp(staticRoots: { uploads: string; outputs: string }) {
  const app = express();
  app.use(cors());
  app.use("/uploads", express.static(staticRoots.uploads));
  app.use("/outputs", express.static(staticRoots.outputs));

  // HTML simples (opcional: servir arquivo index.html)
  app.get("/", (_req, res) => {
    res.type("html").send(`
      <!DOCTYPE html>
<html lang="pt-BR">
...
`);
  ...
  const upload = multer({ dest: staticRoots.uploads });
  routes(app, upload);
